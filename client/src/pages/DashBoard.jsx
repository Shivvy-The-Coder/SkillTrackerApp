...
//                     <div className="h-15 w-15 sm:w-20 sm:h-20 bg-gradient-to-br from-cyan-400 via-purple-500 to-pink-500 rounded-2xl flex items-center justify-center shadow-xl">
...
//                     <p className="text-[11px] text-slate-400">{userData?.email || 'user@example.com'}</p>
//                   </div>
//                 </div>
...
import { AppContext } from '../context/AppContext.jsx';
import { useNavigate } from 'react-router-dom';
import { FaArrowLeft, FaClock, FaTrophy, FaChartLine } from "react-icons/fa";
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="bg-gray-800 px-6 py-4 flex items-center justify-between shadow-md">
        <div className="flex items-center space-x-4">
          <button onClick={() => navigate('/')} className="text-white text-2xl"><FaArrowLeft /></button>
          <div>
      ...
      {/* Stats Cards */}
      <div className="max-w-6xl mx-auto px-6 py-6 grid grid-cols-1 sm:grid-cols-4 gap-4">
        <div className="bg-gray-800 rounded-lg p-4 flex items-center space-x-4 shadow-md hover:scale-105 transition-transform">
        ...
        <div className="bg-gray-800 rounded-lg p-4 flex items-center space-x-4 shadow-md hover:scale-105 transition-transform">
          ...
          <div>
            <p className="text-gray-400 text-sm">Avg Hours/Skill</p>
        ...
        <div className="bg-gray-800 rounded-lg p-4 flex items-center space-x-4 shadow-md hover:scale-105 transition-transform">
          ...
          <div>
            ...
            <p className="text-xl font-bold">{topSkill?.name || 'N/A'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Personal Info */}
        <div className="lg:col-span-1 bg-gray-800 p-6 rounded-lg shadow-md">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold">Profile</h2>
            <button onClick={() => setShowPersonalInfoForm(!showPersonalInfoForm)} className="text-sm text-cyan-400 hover:underline">
              {showPersonalInfoForm ? 'Cancel' : 'Edit'}
            </button>
          </div>
          <div className="mb-4">
          ...
          <div className="mb-4">
            <p className="text-gray-400 font-semibold">Goals</p>
            <p>{goals || 'No goals set yet.'}</p>
          </div>
          {showPersonalInfoForm && (
            <div className="space-y-3">
              <textarea value={bio} onChange={(e) => setBio(e.target.value)} placeholder="Your bio..." className="w-full p-2 rounded bg-gray-700" />
              <textarea value={goals} onChange={(e) => setGoals(e.target.value)} placeholder="Your goals..." className="w-full p-2 rounded bg-gray-700" />
              <button onClick={updatePersonalInfo} className="w-full py-2 bg-gradient-to-r from-cyan-500 to-blue-500 rounded hover:opacity-90 transition-all">Save</button>
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-2 bg-gray-800 p-6 rounded-lg shadow-md">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-2 mb-6">
            <input type="text" placeholder="Skill Name" value={newSkill.name} onChange={(e) => setNewSkill({ ...newSkill, name: e.target.value })} className="p-2 rounded bg-gray-700"/>
            <select value={newSkill.proficiency} onChange={(e) => setNewSkill({ ...newSkill, proficiency: Number(e.target.value) })} className="p-2 rounded bg-gray-700">
              {proficiencyLevels.map(level => (
                <option key={level.value} value={level.value}>{level.label}</option>
              ))}
            </select>
            <input type="number" placeholder="Hours Spent" value={newSkill.hoursSpent} onChange={(e) => setNewSkill({ ...newSkill, hoursSpent: Number(e.target.value) })} className="p-2 rounded bg-gray-700"/>
            <button onClick={addSkill} className="bg-gradient-to-r from-cyan-500 to-blue-500 rounded hover:opacity-90 transition-all">Add</button>
          </div>
          ...
          {skills.length === 0 ? (
            ...
            <div className="space-y-3">
              {skills.map(skill => (
                <div key={skill._id} className="bg-gray-700 p-3 rounded flex flex-col md:flex-row justify-between items-start md:items-center hover:bg-gray-600 transition-all">
                  {editingSkill === skill._id ? (
                    <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 w-full">
                      <input type="text" value={editSkillData.name} onChange={(e) => setEditSkillData({ ...editSkillData, name: e.target.value })} className="p-1 rounded bg-gray-600 flex-1"/>
                      <select value={editSkillData.proficiency} onChange={(e) => setEditSkillData({ ...editSkillData, proficiency: Number(e.target.value) })} className="p-1 rounded bg-gray-600 w-32">
                        {proficiencyLevels.map(level => (
                          <option key={level.value} value={level.value}>{level.label}</option>
                        ))}
                      </select>
                      <input type="number" value={editSkillData.hoursSpent} onChange={(e) => setEditSkillData({ ...editSkillData, hoursSpent: Number(e.target.value) })} className="p-1 rounded bg-gray-600 w-24"/>
                      <div className="flex space-x-2">
                        <button onClick={() => updateSkill(skill._id)} className="bg-green-500 rounded px-2">Save</button>
                        <button onClick={cancelEditing} className="bg-red-500 rounded px-2">Cancel</button>
                      </div>
                    </div>
                  ) : (
                    <div className="flex flex-col sm:flex-row justify-between w-full items-start sm:items-center">
                      <div className="mb-2 sm:mb-0">
                        ...
                        <p className="text-gray-300 text-sm">{proficiencyLevels.find(l => l.value === skill.proficiency)?.label} • {skill.hoursSpent}h</p>
                        <div className="w-full bg-gray-600 h-2 mt-1 rounded">
                          <div className="bg-cyan-400 h-2 rounded" style={{ width: `${(skill.hoursSpent / Math.max(...skills.map(s => s.hoursSpent || 1))) * 100}%` }}></div>
                        </div>
                      </div>
                      <div className="flex space-x-2 mt-2 sm:mt-0">
                        <button onClick={() => startEditingSkill(skill)} className="text-cyan-400 hover:underline text-sm">Edit</button>
                        <button onClick={() => deleteSkill(skill._id)} className="text-red-400 hover:underline text-sm">Delete</button>
                      </div>
                    </div>
                  )}
                </div>
